function uploadFile() {
    ...
    if (file) {
        ...
        fetch('http://149.100.157.235:9000/uploadfile/', {
            method: 'POST',
            body: formData,
        })
...
function renameFile(filename) {
            ...
            if (name !== '') {
                fetch(`http://149.100.157.235:9000/renamefile/${filename}/${name}/`, {
                    method: 'PUT',
                })
                .then(response => response.json())
                .then(data => {
        ...
        async function getFileList() {
            try {
                const response = await fetch('http://149.100.157.235:9000/listfiles/');
        ...
        async function deleteFile(fileName) {
            try {
                const response = await fetch(`http://149.100.157.235:9000/deletefile/${fileName}/`, {
                    method: 'DELETE',
                });
                const data = await response.json();
